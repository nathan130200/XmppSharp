...
{
	private static readonly Dictionary<string, StreamErrorCondition> s_cache = [];
	...
	public static bool operator ==(StreamErrorCondition lhs, StreamErrorCondition rhs) => lhs.Equals(rhs);
	public static bool operator !=(StreamErrorCondition lhs, StreamErrorCondition rhs) => !(lhs == rhs);

	public XElement CreateElement(string? message = default, string? language = "en")
	{
		var element = Namespaces.Stream.CreateElement("error");

		element.C(Value, Namespaces.Streams);

		if (!string.IsNullOrEmpty(message))
		{
			var text = element.C("text", Namespaces.Streams);

			if (!string.IsNullOrWhiteSpace(language))
				text.SetAttributeValue("xml:lang", language);
